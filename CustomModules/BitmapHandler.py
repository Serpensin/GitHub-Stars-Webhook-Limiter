...
class BitmapHandler:
    def __init__(self, keys: List[str]):
        ...
        """
        if len(keys) > 64:
            ValueError("Warning: You are trying to initialize with more than 64 keys. This may exceed the limit for bit manipulation.")
    ...
    def get_active_keys(self, bitkey: int, single: bool = False) -> Union[str, List[str]]:
        ...
        active_keys = [key for key, bit_position in self.bitmap.items() if bitkey & (1 << bit_position)]
...
class BitmapHandler:
    def __init__(self, keys: List[str]):
        ...
        """
        if len(keys) > 64:
            raise ValueError("Warning: You are trying to initialize with more than 64 keys. This may exceed the limit for bit manipulation.")
    ...
    def get_active_keys(self, bitkey: int, single: bool = False) -> Union[str, List[str]]:
        ...
        return active_keys[-1] if single and active_keys else active_keys
    ...
    def toggle_key_in_bitkey(self, key: str, bitkey: int, add: bool = True) -> int:
        ...
        return bitkey | (1 << self.bitmap[key]) if add else bitkey & ~(1 << self.bitmap[key])
